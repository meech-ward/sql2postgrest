...
export type UsePostgREST2SQLResult = {
  convert: (request: PostgRESTInput) => SQLResult | null;
  isLoading: boolean;
  isReady: boolean;
  error: string | null;
  startLoading: () => void;
...
export function usePostgREST2SQL(): UsePostgREST2SQLResult {
  ...
  const convert = useCallback(
    (request: PostgRESTInput): SQLResult | null => {
      if (!isReady || !converter) {
        return null;
      }

      try {
        const result = converter.convertReverse({
        ...
        console.error('[usePostgREST2SQL] Conversion error:', err);
        return null;
      }
    },
    [isReady, converter]
